...
namespace OrderAnydayProject.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(string returnUrl)
        {
            if (System.Net.NetworkInformation.NetworkInterface.GetIsNetworkAvailable())
            {
                if (User.Identity.IsAuthenticated)
                    return RedirectToAction("Profile", "Home", routeValues: null);
            ...
            {
                 return Redirect("~/ErrorPages/500.aspx?");
            }
        ...
        public ActionResult Register()
        {
            ...
            ViewBag.Dept = new SelectList(db.Departments.ToList(), "Name", "Name");
            return View();
        }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var usernamecount = (from s in db.AspNetUsers where s.UserName == model.UserName select s).Count();
                var emailcount = (from s in db.AspNetUsers where s.Email == model.Email select s).Count();
                if (usernamecount > 0)
                {
                    ViewBag.Dept = new SelectList(db.Departments.ToList(), "Name", "Name"); //SelectList(deptList);
                    ViewBag.Name = new SelectList(context.Roles.ToList(), "Name", "Name");
                    ViewData["Error"] = "This UserName already exists. ";
                    return View(model);
                }
                else if (emailcount > 0)
                {
                    ViewBag.Dept = new SelectList(db.Departments.ToList(), "Name", "Name"); //SelectList(deptList);
                    ViewBag.Name = new SelectList(context.Roles.ToList(), "Name", "Name");
                    ViewData["Error"] = "This Email already exists. ";
                    return View(model);
                }
                else
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        TempData["Success"] = "The Profile Was Successfully Created.";//create message on successful Profile creation
            ...
            ViewData["Error"] = "Creation of the profile has failed.";
            return View(model);
        }
